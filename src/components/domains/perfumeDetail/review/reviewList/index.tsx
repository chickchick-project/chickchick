  ...
  return (
    <section className="flex flex-col gap-5">
      ...
      <ul className="flex flex-col tablet:gap-6 tablet:px-5 pc:px-0">
        {data.length > 0 &&
          data.map((review) => {
            const tags = Object.entries(review.chips).flatMap(
              ([category, value]) => {
                const typedCategory = category as ReviewCategory;

                if (Array.isArray(value)) {
                  return value.map((v) => getLabelByKey(typedCategory, v));
                }
                return [getLabelByKey(typedCategory, value)];
              }
            );
            return (
              <li
